...
void VL53L0X::begin(uint8_t i2c_addr = 0x29){
  uint8_t val1;
  delay(1500);
...
void VL53L0X::writeData(unsigned char Reg ,unsigned char *buf, 
	unsigned char Num){
...
void VL53L0X::writeByteData(unsigned char Reg, unsigned char byte){
...
void VL53L0X::readData(unsigned char Reg, unsigned char Num){
...
uint8_t VL53L0X::readByteData(unsigned char Reg){
	uint8_t data;
	Wire.beginTransmission(DetailedData.I2cDevAddr); // transmit to device #8
	Wire.write((uint8_t)Reg);              // sends one byte
	Wire.endTransmission();    // stop transmitting
...
void VL53L0X::setDeviceAddress(uint8_t newAddr){
	newAddr &= 0x7F;
...
void VL53L0X::highPrecisionEnable(FunctionalState NewState){
	writeByteData(VL53L0X_REG_SYSTEM_RANGE_CONFIG, 
		NewState);
}

void VL53L0X::setMode(ModeState mode, PrecisionState precision){
